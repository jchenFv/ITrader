...
当前版本不调用 ChatGPT 或其他大模型 API。它是本地确定性 Agent：新闻关键词情绪评分负责生成候选信号，独立风控规则负责仓位、现金、止损和止盈，模拟券商负责最终成交与记账。相同输入会得到相同结果，适合先验证交易闭环。

策略组件与 Agent、数据源、券商账本已经分离，因此后续可以增加 OpenAI 或其他 API 驱动的策略实现。建议让模型只输出带置信度和依据的结构化候选信号，所有订单继续强制经过本地风控层，而不是让模型绕过风控直接下单。
...
```powershell
$env:PYTHONPATH = "src"
python -m itrader gui --initial-cash 100000
```
...
点击“运行一轮模拟”会在后台抓取新闻和行情；开启“自动”后按照 `--interval` 周期运行。所有按钮都只操作本地模拟账户，窗口中的“纯模拟”标志用于明确区分实盘。
